...
app.delete('/question/:id', async (req, res) => {
    try {
        ...
        if (!includesInArr(questions, id)) {
            const result = failure('Cannot delete a question', `Unable to find a question with an id of ${id}`);
...
app.patch('/question', async (req, res) => {
    try {
        ...
        const questions = db.data.questions;
        if (!includesInArr(questions, id)) {
            const result = failure('Cannot update question', `Unable to find a question with an id of ${id}`);
            return res.send(result);
        }

        const itemIndex = questions.findIndex(q => q.id === id);
        const result = success('Successfully updated question', {index: itemIndex})
        res.send(result);
    } catch (err) {
